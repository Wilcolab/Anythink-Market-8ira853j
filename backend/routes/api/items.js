...
const getFromCache = async (itemId) => {
  const cached = await client.get(itemId);
  if (cached) {
...
// return a item
router.get("/:itemId", auth.optional, async function (req, res, next) {
  const itemId = req.params.itemId;
  const cached = await getFromCache(itemId);
  if (cached) {
    return res.json({ item: cached });
  }

  const item = await Item.findOne({ slug: req.params.itemId }).populate(
    "seller"
  );
  if (!item) {
    return res.sendStatus(404);
