...
class Customer:
    ...
    def __init__(self, customer_id: str, name: str, email: str, phone: str | None = None, address: str | None = None) -> None:
    ...
    def get_contact_info(self) -> dict[str, str]:
        contact: dict[str, str] = {"email": self.email}
        if self.phone:
...
class InventoryManager:
    ...
    def update_product_stock(self, product_id: str, quantity_change: int) -> int:
        product = self.get_product(product_id)
        if not product:
            raise ValueError("Product not found")
        return product.update_stock(quantity_change)

    def search_products(self, category: str | None = None, min_price: float | None = None, max_price: float | None = None) -> list[Product]:
    ...
    def create_order(self, customer_id: str, items_data: list[OrderItemData]) -> Order:
    ...
    def generate_inventory_report(self) -> dict[str, int | float | list[str]]:
        total_products: int = len(self.products)
        total_value: float = sum(p.price * p.stock_quantity for p in self.products.values())
        low_stock: list[Product] = self.get_low_stock_products()
...
class SalesAnalytics:
    ...
    def calculate_revenue(self, start_date: datetime | None = None, end_date: datetime | None = None) -> float:
        total: float = 0
    ...
    def get_top_selling_products(self, limit: int = 10) -> list[dict[str, str | int]]:
    ...
    def get_customer_analytics(self, customer_id: str) -> dict[str, str | int | float] | None:
        customer = self.inventory.customers.get(customer_id)
        if not customer:
            return None

        customer_orders: list[Order] = [o for o in self.inventory.orders.values() if o.customer.customer_id == customer_id]
        total_spent: float = sum(o.calculate_total() for o in customer_orders if o.status != "cancelled")
